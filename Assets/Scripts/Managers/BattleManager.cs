...
public class BattleManager : MonoBehaviour
{
    ...
    public GameObject EnemyInExplore;
    ...
    public void StartBattle(List<GameObject> thePlayers, GameObject[] theEnemies, int goldAmount, List<Collectable> itemAwards, GameObject EnemyInExplore) {
        
        if (!isBattleActive) {
            //Debug.Log("BattleManager: Start Battle");

            this.EnemyInExplore = EnemyInExplore;

            //playerInventory.inventory.AddList(itemAwards);

            List<Collectable> consumableItems = new List<Collectable>();

            foreach (var item in playerInventory.inventory.slots)
            {
                if (item != null && item.itemData != null && item.itemData.isConsumable)
                {
                    consumableItems.Add(new Collectable(item.itemData, item.count));
                }
            }

            uiManager.potionItems = consumableItems;
            ...
            isBattleActive = true;
            mainCamera.transform.position = new Vector3(-25,-26,-10);
            ...
            EnemyInExplore.GetComponent<Enemy>().character = enemies[0].GetComponent<Character>();
    ...
    public void StartNextTurn() {
        ...
        do
        {
            ...
            if (attempts >= maxAttempts) {
                Debug.LogError("Infinite Loop Prevented: No valid characters left to take a turn.");
                return;
            }
    ...
    private void showResults(bool playerWon) {
        ...
        addEXP((int) (uiManager.goldAmount / 5.5));
        
        uiManager.actionButtonPanel.SetActive(false);
    }

    public void addEXP(int exp) {
        Debug.Log("BattleManager: Add EXP");
        int levelsIncreased = gm.allPlayers[0].characterData.UpdateExp(exp);
    ...
    private void UpdateEXP(int levels) {
        for (int j = 0; j < levels; j++) {
            for (int i = 0; i < gm.allPlayers.Count; i++) {
                gm.allPlayers[i].characterData.baseAttack = (int) (gm.allPlayers[i].characterData.baseAttack * 1.3);
                gm.allPlayers[i].characterData.maxHealth = (int) (gm.allPlayers[i].characterData.maxHealth * 1.1);
            }
        }

        Debug.Log($"BattleManager: Player EXP increased by {levels} levels");
    }
