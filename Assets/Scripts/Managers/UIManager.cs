...
public class UIManager : MonoBehaviour
{
    ...
    public List<Collectable> potionItems = new List<Collectable>();
    ...
    public void DisplayItemButtons(Character character) {
        foreach (Transform child in itemsContainer) {
            Destroy(child.gameObject);
        }

        foreach (var potion in potionItems) {
            GameObject button = Instantiate(actionButtonPrefab, itemsContainer);
            TMP_Text buttonText = button.GetComponentInChildren<TMP_Text>();
            buttonText.text = potion.itemData.itemName;
            Button btn = button.GetComponent<Button>();
            btn.onClick.AddListener(() => {
                character.BattleHeal(potion.itemData.healAmount);
                potion.count--;
                bm.playerInventory.inventory.Remove(potion.itemData, 1);
                character.TU += 100;
        ...
        ShowEnemySelection(false);
        attackButtonContainer.gameObject.SetActive(false);
    ...
    public void UpdateResultsScreen() {
        ...
        continueButton.onClick.AddListener(() => {
            bm.EndBattle(true);
            bm.EnemyInExplore.GetComponent<Enemy>().ShowAfterBattleDialogue();
            Destroy(bm.EnemyInExplore);
            bm.EnemyInExplore = null;
        });
    }
